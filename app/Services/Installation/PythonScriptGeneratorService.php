...
class PythonScriptGeneratorService
{
    private ?string $templateOverride = null;

    /**
     * Set a template override for testing purposes
     */
    ...
    /**
     * Generate a Python installation script for Windows.
     */
    public function generateWindowsScript(
        ...
        // Get template
        $scriptTemplate = $this->getWindowsTemplate();
    ...
    /**
     * Get the Windows Python script template.
     */
    private function getWindowsTemplate(): string
    {
        // If we have an override template (for testing), use it
        if ($this->templateOverride !== null) {
            return $this->templateOverride;
        }

        try {
            $templatePath = resource_path('scripts/install-agent-python-windows.template.py');
            if (File::exists($templatePath)) {
                return File::get($templatePath);
            }
        } catch (Exception $e) {
            // If there's any error accessing the file, fall back to default template
        }

        return $this->getDefaultWindowsTemplate();
    }

    /**
     * Get the default Windows Python script template.
     */
    private function getDefaultWindowsTemplate(): string
    {
        return <<<'PYTHON'
#!/usr/bin/env python3
...
def download_agent() -> str:
    ...
    try:
        with urllib.request.urlopen(agent_url) as response, open(agent_path, 'wb') as out_file:
...
def install_windows_service(agent_path: str) -> None:
    ...
    subprocess.run(['sc', 'create', 'ComputerAgent',
                    'binPath=', agent_path, 'start=', 'auto',
                    'DisplayName=', 'Computer Management Agent'],
                    check=True, capture_output=True)

    subprocess.run(['sc', 'description', 'ComputerAgent',
                    'Allows remote management of this computer'],
                    check=True, capture_output=True)

    subprocess.run(['sc', 'start', 'ComputerAgent'],
                    check=True, capture_output=True)
...
def register_computer() -> None:
    ...
    try:
        ...
        req = urllib.request.Request(registration_url, data=data, headers=headers, method='POST')
        ...
        print("The agent is installed and will connect, but registration must be completed manually.")
